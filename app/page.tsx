...
const faqs=[
  ...
  ['Why is the minimum stay 32 nights?','32+ is designed specifically for medium-term stays. Every published home has a minimum stay of at least 32 nights.'],
...
export default function HomePage(){
  ...
  function submit(event:FormEvent){event.preventDefault();const params=new URLSearchParams({where,guests});if(moveIn)params.set('moveIn',moveIn);if(moveOut)params.set('moveOut',moveOut);window.location.href='/homes?'+params}
...
function HomeCollection({title,description,homes:collection}:{title:string;description:string;homes:Home[]}){return <section className="market-collection"><div className="collection-title"><div><h2>{title}</h2><p>{description}</p></div><Link href="/homes">View all homes →</Link></div><div className="market-cards">{collection.map(home=><Link href={`/home/${home.id}`} className="market-card" key={home.id}><div className="market-card-image" style={{backgroundImage:`url(${home.image})`}}><span>Request only</span><b aria-label="Save home">♡</b></div><div className="market-card-copy"><div><h3>{home.title}</h3><p>{home.location}</p></div><strong>€{home.price.toLocaleString()} <small>/ month</small></strong></div></Link>)}</div></section>}
