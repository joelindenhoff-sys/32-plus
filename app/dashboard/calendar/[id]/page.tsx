...
function monthKeys():Month[]{const now=new Date();return Array.from({length:12},(_,i)=>{const d=new Date(now.getFullYear(),now.getMonth()+i,1);return{key:`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`,label:new Intl.DateTimeFormat('en-GB',{month:'long',year:'numeric'}).format(d),year:d.getFullYear(),month:d.getMonth()}})}
...
export default function CalendarSettings(){
  ...
  return <><Header/><main className="owner-tool-page calendar-page"><div className="owner-tool-header"><div><Link href="/dashboard">← Back to dashboard</Link><p className="eyebrow">CALENDAR & PRICING</p><h1>{property.title}</h1></div><Link className="outline-action" href={`/dashboard/property/${id}`}>Edit property</Link></div><form onSubmit={save}>
    <section className="calendar-settings-card"><h2>Availability and fees</h2><div className="settings-grid"><label>Cleaning fee (€)<input type="number" min="0" value={property.cleaning_fee} onChange={e=>set('cleaning_fee',e.target.value)}/></label><label>Minimum stay<input type="number" min="32" value={property.minimum_nights} onChange={e=>set('minimum_nights',e.target.value)}/><span>Cannot be lower than 32 nights.</span></label><label>Cancellation policy<select value={property.cancellation_policy} onChange={e=>set('cancellation_policy',e.target.value)}><option value="flexible">Flexible</option><option value="moderate">Moderate</option><option value="firm">Firm</option><option value="non_refundable">Non-refundable</option></select></label></div></section>
